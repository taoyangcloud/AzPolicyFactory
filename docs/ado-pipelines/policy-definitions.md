...
1. After the `Initiation` stage, the `Policy Test` stage performs a number of predefined Pester tests for each policy definition file that are to be deployed.
...
3. The `Test Dev` and `Test Prod` stages are responsible for perform additional tests in their respective environments. They are kicked off after the `Build Dev` and `Build Prod` stages respectively.
...
These stages uses the pipeline template [template-stage-policy-def-build.yml](../../.azuredevops/templates/template-stage-policy-def-build.yml) to populate the paths of each policy definition JSON files and add them to the Policy Definition Bicep template file.
...
These stages uses the pipeline template [template-job-test-and-validate.yml](../../.azuredevops/templates/template-job-test-and-validate.yml) to perform a set of tests on the Bicep templates generated in the `Build Dev` and `Build Prod` stages respectively.
