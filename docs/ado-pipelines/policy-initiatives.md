...
1. After the `Initiation` stage, the `Policy Test` stage performs a number of predefined Pester tests for each policy initiative file that are to be deployed.
...
These stages uses the pipeline template [template-stage-policy-def-build.yml](../../.azuredevops/templates/template-stage-policy-def-build.yml) to populate the paths of each policy definition JSON files and add them to the Policy Initiative Bicep template file.

These JSON files which will then get loaded at compile time by the Policy Initiative bicep module using the `LoadJsonContent()` Bicep function.
