...
These jobs uses the custom action [build-policy-assignment-and-exemption](../../.github/actions/templates/build-policy-assignment-and-exemption/action.yml) to populate the paths of each policy exemption configuration file and add them to the Policy exemption Bicep template file.

The workflow evaluates each policy exemption configuration file and filter out the ones that are already expired because there is no point to deploy expired exemptions.
