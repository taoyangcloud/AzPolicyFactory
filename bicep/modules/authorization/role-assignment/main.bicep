...
output name string = (empty(subscriptionId) && empty(resourceGroupName) ? roleAssignment_mg.?outputs.name : null) ?? (!empty(subscriptionId) && empty(resourceGroupName)
  ? roleAssignment_sub.?outputs.name
  : null) ?? roleAssignment_rg.?outputs.name ?? ''

@sys.description('The resource ID of the Role Assignment.')
output resourceId string = (empty(subscriptionId) && empty(resourceGroupName)
  ? roleAssignment_mg.?outputs.?resourceId
  : null) ?? (!empty(subscriptionId) && empty(resourceGroupName) ? roleAssignment_sub.?outputs.resourceId : null) ?? roleAssignment_rg.?outputs.resourceId ?? ''

@sys.description('The scope this Role Assignment applies to.')
output scope string = (empty(subscriptionId) && empty(resourceGroupName) ? roleAssignment_mg.?outputs.scope : null) ?? (!empty(subscriptionId) && empty(resourceGroupName)
  ? roleAssignment_sub.?outputs.?scope
  : null) ?? roleAssignment_rg.?outputs.?scope ?? ''
