...
AUTHOR: Tao Yang
DATE: 01/05/2024
NAME: pipeline-template-deployment-rest.ps1
VERSION: 1.0.0
...
param (
  ...
  [parameter(Mandatory = $false)]
  [boolean]$publishDeploymentOutputs = $true,
...
$deploymentAttempt = 1
$templateFile = join-path -path $SystemDefaultWorkingDirectory -Childpath $templateFileDirectory -AdditionalChildPath $templateFileName
...
if ($parameterFileName -ine '') {
  If ( $parameterFileArtifactName -ne "") {
    $ParameterFileLocation = join-path -path $parameterFileDirectory -Childpath $parameterFileArtifactName -AdditionalChildPath $parameterFileName
    Write-Output "[$(GetCurrentUTCString)]: Deploying with Parameter File '$ParameterFileLocation' from Build Artifact..."
  } else {
    $ParameterFileLocation = join-path -path $parameterFileDirectory -ChildPath $parameterFileName
...
if ($deploymentOutputs) {
  ...
  if ($publishDeploymentOutputs) {
    Write-Output "[$(GetCurrentUTCString)]: Saving Deployment Outputs..."

    #Save deployment output to File
    $deploymentOutputDir = "$SystemDefaultWorkingDirectory\$templateFileDirectory\Outputs"
    ...
    Write-Output "##vso[task.setvariable variable=deploymentName]$deploymentName"
    Write-Output "##vso[task.setvariable variable=deploymentName;isOutput=true]$deploymentName"
  } else {
  ...
  if ($deploymentOutputVariablePrefix.Length -gt 0) {
    foreach ($key in $($deploy.outputs.keys)) {
      ...
      Write-Output "##vso[task.setvariable variable=$pipelineVariableName]$pipelineVariableValue"
      Write-Output "##vso[task.setvariable variable=$pipelineVariableName;isOutput=true]$pipelineVariableValue"
    }
  } else {
...
Write-Output "##vso[task.setvariable variable=deploymentOutputPublished]$deploymentOutputPublished"
Write-Output "##vso[task.setvariable variable=deploymentOutputPublished;isOutput=true]$deploymentOutputPublished"
