...
AUTHOR: Tao Yang
DATE: 27/02/2025
NAME: Invoke-TemplateValidation.ps1
VERSION: 1.1.0
COMMENT: Bicep template validation script used in GitHub Actions
=================================================================
#>
...
$WhatIfvalidationHelperScriptPath = Join-Path $PSScriptRoot 'Invoke-ArmWhatIfValidation.ps1'
...
If ($runWhatIfValidation -eq 'true') {
  ...
  $whatIfResultJson = $whatIfResult | ConvertTo-Json -Depth 99
} else {
...
If ($runWhatIfValidation -eq 'true') {
  if ($whatIfResultArtifactName.length -ge 1) {
    ...
    New-Item -Path $whatIfResultDir -ItemType Directory -Force | Out-Null
